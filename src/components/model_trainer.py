...
from src.components.model_arch import XLMRobertaForTokenClassification
...
class TrainTokenClassifier:
    ...
    def train(self):
        try:
            ...
            logging.info(" Training Running ")
            result = trainer.train()
            logging.info(f" Result of the training {result} ")
            trainer.save_model(self.model_training_config.output_dir)
            logging.info(f" Model Saved at [{self.model_training_config.output_dir}]")
        except Exception as e:
            logging.exception(e)
            raise e
